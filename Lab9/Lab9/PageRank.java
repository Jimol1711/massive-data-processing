	...
	public void compute(Vertex<Text, DoubleWritable, NullWritable> vertex,
		...
		if(getSuperstep() == 0) {
			...
			if(getSuperstep() == ITERS+2) {
				...
				if(edges > 0) {
					// TODO: compute value to send to each neighbour
					sendMessageToAllEdges(vertex, new DoubleWritable(currentRank * D / edges));
					
					// TODO: send remaining rank to random jump probability aggregator
					aggregate(RJ_AGG_NAME, new DoubleWritable(currentRank * (1 - D)));
				} else {
					// TODO: send rank to random jump probability aggregator
